...
namespace Core.UI
{
    public partial class FormSpinner1Teach : Form
    {
        ...
        private void TimerUI_Tick(object sender, EventArgs e)
        {
            ...
            strCurPos = String.Format("{0:0.0000}", CMainFrame.Core.m_YMC.ServoStatus[(int)Axis_CleanNozzle_T].EncoderPos);
            GridNozzleTeachTable[7, 1].Text = strCurPos;

            strCurPos = String.Format("{0:0.0000}", CMainFrame.Core.m_YMC.ServoStatus[(int)Axis_CoatNozzle_T].EncoderPos);
            GridNozzleTeachTable[7, 2].Text = strCurPos;

            strCurPos = String.Format("{0:0.0000}", CMainFrame.Core.m_YMC.ServoStatus[(int)Axis_Rotate_T].EncoderPos);
            GridRotateTeachTable[7, 1].Text = strCurPos;

            // 보정값 Display
            double dValue = 0, dCurPos = 0, dTargetPos = 0;

            dCurPos = CMainFrame.Core.m_YMC.ServoStatus[(int)Axis_CleanNozzle_T].EncoderPos;
            ...
            dCurPos = CMainFrame.Core.m_YMC.ServoStatus[(int)Axis_CoatNozzle_T].EncoderPos;
            dTargetPos = Convert.ToDouble(GridNozzleTeachTable[2, 2].Text);
            dValue = dTargetPos - dCurPos;

            GridNozzleTeachTable[8, 2].Text = String.Format("{0:0.0000}", dValue);

            dCurPos = CMainFrame.Core.m_YMC.ServoStatus[(int)Axis_Rotate_T].EncoderPos;
