...
namespace Core.UI
{
    public partial class FormLoaderTeach : Form
    {
        ...
        private CMovingObject MO_Loader = CMainFrame.Core.m_MeElevator.AxElevatorInfo;
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            EPositionObject pIndex = EPositionObject.LOADER;
            int direction = DEF_Z;
        ...
        private void TimerUI_Tick(object sender, EventArgs e)
        {
            ...
            strCurPos = String.Format("{0:0.0000}", CMainFrame.Core.m_YMC.ServoStatus[(int)EYMC_Axis.LOADER_Z].EncoderPos);
            GridTeachTable[7, 1].Text = strCurPos;

            // 보정값 Display
            double dValue = 0, dCurPos = 0, dTargetPos = 0;

            dCurPos = CMainFrame.Core.m_YMC.ServoStatus[(int)EYMC_Axis.LOADER_Z].EncoderPos;
        ...
        private void DisplayPos()
        {
            ...
            dTargetPos = MO_Loader.GetPosition(index, DEF_Z, out dFixedPos, out dModelPos, out dOffsetPos, out dAlignOffset);
        ...
        private void BtnTeachMove_Click(object sender, EventArgs e)
        {
            ...
            bool bDetected;
            int iResult = CMainFrame.Core.m_ctrlLoader.IsWaferDetected(out bDetected);
            if (iResult != SUCCESS)
            {
            ...
            switch(index)
            {
                case (int)EElevatorPos.BOTTOM:
                    iResult = CMainFrame.Core.m_ctrlLoader.MoveToBottomPos();
                    break;
                case (int)EElevatorPos.LOAD:
                    iResult = CMainFrame.Core.m_ctrlLoader.MoveToLoadPos();
                    break;
                case (int)EElevatorPos.TOP:
                    iResult = CMainFrame.Core.m_ctrlLoader.MoveToTopPos();
                    break;
                case (int)EElevatorPos.SAFETY:
                    iResult = CMainFrame.Core.m_ctrlLoader.MoveToSafetyPos();
                    break;
                case (int)EElevatorPos.SLOT:
                    iResult = CMainFrame.Core.m_ctrlLoader.MoveToSlotPos();
                    break;
