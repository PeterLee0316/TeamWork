    ...
    public partial class FormManualScreen : Form
    {
        ...
        private async void Btn_Click(object sender, EventArgs e)
        {
            ...
            if (unit == "Loader")
            {
                if(cmd == "Bottom")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlLoader.MoveToBottomPos());
                    iResult = await task1;
                }
                else if (cmd == "Top")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlLoader.MoveToTopPos());
                    iResult = await task1;
                }
                else if (cmd == "Slot")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlLoader.MoveToSlotPos());
                    iResult = await task1;
                }
                else if (cmd == "Load")
                {
            ...
            {
                iResult = CMainFrame.Core.m_ctrlPushPull.IsObjectDetected(out bStatus);
                ...
                if (cmd == "Wait")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToWaitPos(bTransfer));
                    iResult = await task1;
                }
                else if (cmd == "Load")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToLoaderPos(bTransfer));
                    iResult = await task1;
                }
                else if (cmd == "Spinner 1")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToSpinner1Pos(bTransfer));
                    iResult = await task1;
                }
                else if (cmd == "Spinner 2")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToSpinner2Pos(bTransfer));
                    iResult = await task1;
                }
                else if (cmd == "Handler")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToHandlerPos(bTransfer));
                    iResult = await task1;
                }
                else if (cmd == "Temp Unload")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToTempUnloadPos(bTransfer));
                    iResult = await task1;
                }
                else if (cmd == "Reload")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveToReloadPos(bTransfer));
                    iResult = await task1;
                }
            }
            else if (unit == "Centering")
            {
                if (cmd == "Open")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveAllCenterUnitToWaitPos());
                    iResult = await task1;
                }
                else if (cmd == "Close")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlPushPull.MoveAllCenterUnitToCenteringPos());
                    iResult = await task1;
                }
            ...
            else if (unit == "Spinner1_CoatNozzle")
            {
                if (cmd == "Safety")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveCoatNozzleToSafetyPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing Start")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveCoatNozzleToStartPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing End")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveCoatNozzleToEndPos());
                    iResult = await task1;
                }
            }
            else if (unit == "Spinner1_CleanNozzle")
            {
                if (cmd == "Safety")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveCleanNozzleToSafetyPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing Start")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveCleanNozzleToStartPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing End")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveCleanNozzleToEndPos());
                    iResult = await task1;
                }
            }
            else if (unit == "Spinner1_Rotate")
            {
                if (cmd == "Home")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner1.MoveRotateToLoadPos());
                    iResult = await task1;
                }
            }
            ///////////////////////////////////////////////////////////////////////////////
            ...
            else if (unit == "Spinner2_CoatNozzle")
            {
                if (cmd == "Safety")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveCoatNozzleToSafetyPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing Start")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveCoatNozzleToStartPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing End")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveCoatNozzleToEndPos());
                    iResult = await task1;
                }
            }
            else if (unit == "Spinner2_CleanNozzle")
            {
                if (cmd == "Safety")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveCleanNozzleToSafetyPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing Start")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveCleanNozzleToStartPos());
                    iResult = await task1;
                }
                else if (cmd == "Dispensing End")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveCleanNozzleToEndPos());
                    iResult = await task1;
                }
            }
            else if (unit == "Spinner2_Rotate")
            {
                if (cmd == "Home")
                {
                    task1 = Task<int>.Run(() => CMainFrame.Core.m_ctrlSpinner2.MoveRotateToLoadPos());
                    iResult = await task1;
                }
            }
            ///////////////////////////////////////////////////////////////////////////////
