...
namespace Core.UI
{
    public partial class FormAutoScreen : Form
    {
        ...
        private void StartAutoRun()
        {
            ...
            CSystemData systemData = CMainFrame.Core.m_DataManager.SystemData;
            iResult = CMainFrame.Core.m_ctrlOpPanel.CheckDoorSafety(out bStatus);
            if(iResult != SUCCESS)
            {
                if (bStatus == true)
                {
            ...
            iResult = CMainFrame.Core.m_ctrlOpPanel.CheckAreaSafety(out bStatus);
            if (iResult != SUCCESS)
            {
                if (bStatus == true)
                {
            ...
            if (CMainFrame.Core.CheckSystemConfig_ForRun(out strErr) == false)
            {
            ...
            //        if (iResult)
            //        {
            //            strErr = CMainFrame.Core.GetErrorMessage(iResult);
            ...
            bool[] bInitSts;
            if (!CMainFrame.Core.m_OpPanel.CheckAllInit(out bInitSts))
            {
            ...
            // 자동운전 사전조건 확인
            if ((iResult = CMainFrame.Core.m_ctrlOpPanel.CheckBeforeAutoRun()) != SUCCESS)
            {
            ...
            //m_nStartReady = 1;
            //SetButtonStatus(true);
            CMainFrame.Core.m_trsAutoManager.SendMsg(EThreadMessage.MSG_READY_RUN_CMD);

            // Message Pop up
            // RUN SW 또는 Stop SW 눌릴 때 까지 대기
#if SIMULATION_TEST
            //m_nStartReady = 2;
            //CMainFrame.Core.Sleep(1000);
            //CMainFrame.Core.m_trsAutoManager.SendMsg(EThreadMessage.MSG_START_CMD);
#else
        ...
        private void StopAutoRun()
        {
            ...
            //m_nStartReady = 0;
            //SetButtonStatus(false);
            CMainFrame.Core.m_trsAutoManager.SendMsg(EThreadMessage.MSG_CYCLE_STOP_CMD);
        }
        ...
        private void TimerUI_Tick(object sender, EventArgs e)
        {
            ...
            Label_StatusAutoManager.Text = CMainFrame.Core.m_trsAutoManager.GetRunStatus().Replace("STS_", "");
            Label_StatusLoader.Text = CMainFrame.Core.m_trsLoader.GetRunStatus().Replace("STS_", "");
            Label_StatusPushPull.Text = CMainFrame.Core.m_trsPushPull.GetRunStatus().Replace("STS_", "");
            Label_StatusSpinner1.Text = CMainFrame.Core.m_trsSpinner1.GetRunStatus().Replace("STS_", "");
            Label_StatusSpinner2.Text = CMainFrame.Core.m_trsSpinner2.GetRunStatus().Replace("STS_", "");
            Label_StatusUHandler.Text = CMainFrame.Core.m_trsHandler.GetRunStatus().Replace("STS_", "");
            Label_StatusLHandler.Text = CMainFrame.Core.m_trsHandler.GetRunStatus().Replace("STS_", "");
            Label_StatusStage1.Text = CMainFrame.Core.m_trsStage1.GetRunStatus().Replace("STS_", "");
            ...
            Label_StepLoader.Text = CMainFrame.Core.m_trsLoader.GetStep().Replace("TRS_LOADER_", "");
            Label_StepPushPull.Text      = CMainFrame.Core.m_trsPushPull.GetStep().Replace("TRS_PUSHPULL_", "");
            Label_StepSpinner1.Text      = CMainFrame.Core.m_trsSpinner1.GetStep().Replace("TRS_SPINNER_", "");
            Label_StepSpinner2.Text      = CMainFrame.Core.m_trsSpinner2.GetStep().Replace("TRS_SPINNER_", "");
            Label_StepUHandler.Text      = CMainFrame.Core.m_trsHandler.GetStep1().Replace("TRS_UPPER_HANDLER_", "");
            Label_StepLHandler.Text      = CMainFrame.Core.m_trsHandler.GetStep2().Replace("TRS_LOWER_HANDLER_", "");
            Label_StepStage1.Text         = CMainFrame.Core.m_trsStage1.GetStep().Replace("TRS_STAGE1_", "");
