...
namespace LWDicer.UI
{
    public partial class FormSpinner2Teach : Form
    {
        ...
        private void TmrTeach_Tick(object sender, EventArgs e)
        {
            ...
            strCurPos = String.Format("{0:0.000}", CMainFrame.LWDicer.m_YMC.ServoStatus[(int)EYMC_Axis.S2_CLEAN_NOZZLE_T].EncoderPos);
            GridNozzleTeachTable[7, 1].Text = strCurPos;

            strCurPos = String.Format("{0:0.000}", CMainFrame.LWDicer.m_YMC.ServoStatus[(int)EYMC_Axis.S2_COAT_NOZZLE_T].EncoderPos);
            GridNozzleTeachTable[7, 2].Text = strCurPos;

            strCurPos = String.Format("{0:0.000}", CMainFrame.LWDicer.m_YMC.ServoStatus[(int)EYMC_Axis.S2_CHUCK_ROTATE_T].EncoderPos);
